...
export default {
  ...
  data() {
   ...
   computed:{
   selecteddata(){
     return this.$store.state.questiondata.questionsobject.filter(i=>i.question_id==this.$route.params.id)
   }
 },
  async created() {
    try {
  ...
  methods: {
    async storeanswers(event) {
      ...
      console.log('answersdata',this.answersdata.items[event.target.dataset.id]);
